...
interface LoanTypeModalProps {
  ...
  onSuccess: () => void; // Callback to refresh data on the parent page
}
  ...
  useEffect(() => {
    if (isOpen) {
      if (isEditing) {
        ...
        // Reset form when adding a new type
    ...
    <>
      {isOpen && (
        <div className="fixed inset-0 z-50">
          <div className="bg-black/50 animate-overlayShow fixed inset-0" onClick={() => setIsOpen(false)} />
          <div
            role="dialog"
            aria-modal="true"
            aria-labelledby="loan-type-modal-title"
            className="animate-contentShow fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[90vw] max-w-md bg-white p-6 rounded-2xl shadow-lg z-50 focus:outline-none"
            onClick={e => e.stopPropagation()}
          >
            <div id="loan-type-modal-title" className="text-lg font-bold text-navy mb-4">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="flex justify-end gap-4 pt-4">
                <button type="button" onClick={() => setIsOpen(false)} className="px-4 py-2 text-sm font-medium text-gray-700 bg-gray-100 rounded-md hover:bg-gray-200">Cancel</button>
            ...
            <button type="button" onClick={() => setIsOpen(false)} className="absolute top-4 right-4 p-1 rounded-full text-gray-500 hover:bg-gray-100" aria-label="Close">
              <X className="w-4 h-4" />
            </button>
          </div>
        </div>
